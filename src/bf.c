...
bf_program_t parse_program(const char *code) {
    ...
    forall(i, strlen(code) + 1) {
        ...
        switch (c) {
            case TK_LOOP_START: {
                if (prev != '\0') {
                    dynarr_append(&program.code, ((bf_token_t){prev, count}));
                    count = 1;
                }
                // Placeholder
                size_t idx_open = program.code.count;
                dynarr_append(&program.code, ((bf_token_t){TK_LOOP_START, 0}));
            ...
            case TK_LOOP_END: {
                ...
                dynarr_append(&program.code, ((bf_token_t){TK_LOOP_END, idx_open + 1}));
...
void run_program(bf_program_t *program) {
    dynarr_reserve(&program->memory, 32*32);
    uint16_t sp = 0;
    forall(i, program->code.count) {
        bf_token_t token = program->code.items[i];
        switch (token.type) {
            ...
            case TK_OUTPUT: if (!bf_debug) for (__uint128_t j = 0; j < token.data; j++) putchar(program->memory.items[sp]); break;
...
void print_memory(const bf_memory_t *memory) {
